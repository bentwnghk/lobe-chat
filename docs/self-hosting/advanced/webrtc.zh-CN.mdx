---
title: Mr.🆖 AI WebRTC 同步配置指南
description: 在 Mr.🆖 AI 中实现基于 WebRTC 和 YJS 的设备间实时数据同步。了解如何配置 WebRTC 并开启同步功能，以及使用局限性和已知问题。
tags:
  - YJS
  - 信令服务器
---
...
WebRTC (Web Real-Time Communication) 是一项实现浏览器之间点对点通信的技术。在 Mr.🆖 AI 中,我们实验性地基于 WebRTC 和 YJS 实现了设备间的实时数据同步,无需依赖传统的服务器数据库。这种方案具有高度隐私性、零冲突性，并能提供实时会话同步体验。

## 配置 WebRTC 并实现同步

要使用 Mr.🆖 AI 的 WebRTC 同步功能,需要完成以下步骤:
