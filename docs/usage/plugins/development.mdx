---
title: Mr.🆖 AI Plugin Development Guide
description: >-
  Learn how to create and integrate custom plugins in Mr.🆖 AI, including plugin
...
A Mr.🆖 AI plugin consists of the following components:
...
This section will introduce how to add and use a custom plugin in Mr.🆖 AI.
...
### Add Local Plugin in Mr.🆖 AI Role Settings

Next, go to Mr.🆖 AI, create a new assistant, and go to its session settings page:

<Image
  alt="Add Local Plugin in Mr.🆖 AI Role Settings S1"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in Mr.🆖 AI Role Settings S2"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in Mr.🆖 AI Role Settings S3"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in Mr.🆖 AI Role Settings S4"
  height={483}
  inStep
...
After completing these operations, you have understood the basic process of adding custom plugins and using them in Mr.🆖 AI.
...
3. `ui`: This field contains information about the plugin's user interface, indicating from which address Mr.🆖 AI loads the plugin's front-end interface. Since Mr.🆖 AI plugin interface loading is implemented based on iframes, the height and width of the plugin interface can be specified as needed.
4. `gateway`: This field specifies the gateway for Mr.🆖 AI to query the plugin's API interface. Mr.🆖 AI's default plugin gateway is a cloud-based service, and requests for custom plugins need to be sent to a locally launched service. Remote calls to a local address are generally not feasible. The `gateway` field solves this problem. By specifying the gateway in the manifest, Mr.🆖 AI will send plugin requests to this address, and the local gateway address will dispatch requests to the local plugin service. Published online plugins do not need to specify this field.
...
Since the default plugin gateway for Mr.🆖 AI is a cloud service `/api/plugins`, the cloud service sends requests to the address specified in the manifest's `api.url` to solve cross-origin issues.

For custom plugins, plugin requests need to be sent to the local service. Therefore, by specifying the gateway in the manifest ([http://localhost:3400/api/gateway](http://localhost:3400/api/gateway)), Mr.🆖 AI> will directly request this address, and then only the corresponding gateway needs to be created at that address.

```ts
import { createMr.🆖 AIPluginGateway } from '@lobehub/chat-plugins-gateway';

export const config = {
  runtime: 'edge',
};

export default createMr.🆖 AIPluginGateway();
```

[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) contains the implementation of the plugin gateway in Mr.🆖 AI [here](https://github.com/lobehub/lobe-chat/blob/main/src/pages/api/plugins.api.ts). You can use this package directly to create a gateway, allowing Mr.🆖 AI to access the local plugin service.
...
Mr.🆖 AI implements the loading of plugin UI through `iframe` and uses `postMessage` to communicate with the plugin. Therefore, the implementation of the plugin UI is consistent with regular web development. You can use any frontend framework and development language you are familiar with.
...
As for plugin communication, we provide relevant methods in [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) to simplify communication between the plugin and Mr.🆖 AI. You can actively retrieve the current message data from Mr.🆖 AI using the `fetchPluginMessage` method. For detailed information about this method, see: [fetchPluginMessage][fetch-plugin-message-url].
...
const Render = memo(() => {
  ...
  useEffect(() => {
    // Retrieve the current plugin message from Mr.🆖 AI
